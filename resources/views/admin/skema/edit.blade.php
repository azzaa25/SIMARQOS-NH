    ...
    <div class="flex justify-between items-end mb-4">
        <div>
            <nav class="flex text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">
                ...
                <span class="text-green-800">Edit</span>
            </nav>
            <h1 class="text-xl font-bold text-green-900 leading-none">Edit Skema Arisan</h1>
        </div>
        <p class="text-[11px] text-gray-400 italic">Masjid Nurul Huda</p>
    </div>

    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <form id="edit-skema-form" action="{{ route('admin.skema.update', $skema->id_skema) }}"
              method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2 space-y-1">
                    ...
                    <div class="relative">
                        ...
                        <input name="nama_skema" type="text" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-gray-50">
                <div class="space-y-1">
                    ...
                    <select name="durasi_bulan" required
                            class="w-full px-4 py-2 text-sm rounded-lg border border-gray-200
                                   focus:ring-2 focus:ring-green-500/10 focus:border-green-600 outline-none">
                        <option value="12" {{ $skema->durasi_bulan == 12 ? 'selected' : '' }}>
                            1 Tahun (12 Bulan)
                        </option>
                        <option value="36" {{ $skema->durasi_bulan == 36 ? 'selected' : '' }}>
                            3 Tahun (36 Bulan)
                        </option>
                    </select>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <select name="tipe_skema" required
                            class="w-full px-4 py-2 text-sm rounded-lg border border-gray-200
                                   focus:ring-2 focus:ring-green-500/10 focus:border-green-600 outline-none">
                        <option value="perorangan" {{ $skema->tipe_skema == 'perorangan' ? 'selected' : '' }}>
                            Perorangan
                        </option>
                        <option value="kelompok" {{ $skema->tipe_skema == 'kelompok' ? 'selected' : '' }}>
                            Kelompok
                        </option>
                    </select>
                </div>
            ...
            <div class="space-y-1 pt-2 border-t border-gray-50">
                ...
                <textarea name="deskripsi" rows="2"
                          class="w-full px-4 py-2 text-sm rounded-lg border border-gray-200
                                 focus:ring-2 focus:ring-green-500/10 focus:border-green-600 outline-none resize-none transition-all">{{ $skema->deskripsi }}</textarea>
            </div>
...
<script>
    function confirmUpdate() {
        Swal.fire({
            ...
            text: "Pastikan data skema arisan sudah benar sebelum disimpan.",
            icon: 'question',
            iconColor: '#147a54',
            ...
            if (result.isConfirmed) {
                // Menampilkan loading sebelum submit
                Swal.fire({
                    title: 'Menyimpan...',
                ...
                document.getElementById('edit-skema-form').submit();
            }
        });
    }
</script>
