    ...
    <div class="px-4 md:px-0">
        ...
        <div class="flex flex-wrap gap-2 pb-4 border-b border-slate-100">
            ...
            @foreach($kategori as $kat)
                {{-- Memperbaiki slug kategori untuk filter --}}
    ...
    <div class="bg-white relative w-full max-w-5xl h-[85vh] md:h-[80vh] overflow-hidden rounded-[40px] shadow-2xl animate-zoomIn flex flex-col md:flex-row">
        ...
        <div class="md:w-5/12 bg-slate-50 p-6 md:p-10 flex flex-col overflow-y-auto custom-scrollbar border-r border-slate-100">
            ...
            <div class="space-y-4 text-left">
                ...
                {{-- Section Dokumentasi yang akan muncul jika ada datanya --}}
                <div id="section_dokumentasi" class="hidden space-y-4 pt-4 text-left">
                    ...
                    <div id="dokumentasi_container" class="grid grid-cols-1 gap-4">
                        {{-- Foto-foto dokumentasi akan di-inject di sini via JS --}}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="md:w-7/12 p-6 md:p-10 flex flex-col bg-white relative h-full">
            ...
            <div class="flex-1 flex flex-col min-h-0 text-left">
                ...
                <div id="donatur_list" class="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1 bg-slate-50/50 rounded-[30px] p-5 border border-slate-50"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    function filterKategori(slug, btn) {
        ...
        cards.forEach(card => {
            const isMatch = (slug === 'all' || card.getAttribute('data-category') === slug);
    ...
    function loadDetail(item, terkumpulFormatted, donaturArray) {
        ...
        // Handle Dokumentasi (Array of images)
        const sectionDok = document.getElementById('section_dokumentasi');
        const dokContainer = document.getElementById('dokumentasi_container');
        dokContainer.innerHTML = ''; // Clear previous images

        if (item.dokumentasi && Array.isArray(item.dokumentasi) && item.dokumentasi.length > 0) {
            sectionDok.classList.remove('hidden');
            item.dokumentasi.forEach(path => {
                const img = document.createElement('img');
                img.src = "{{ asset('storage') }}/" + path;
                img.className = "rounded-3xl w-full object-cover border-4 border-white shadow-md mb-2";
                dokContainer.appendChild(img);
            });
        } else {
        ...
        if(donaturArray.length > 0) {
            donaturArray.forEach(d => {
                ...
                row.innerHTML = `
                    ...
                    <div class="text-right"><p class="text-xs font-black text-[#147a54]">Rp ${new Intl.NumberFormat('id-ID').format(d.nominal)}</p></div>`;
