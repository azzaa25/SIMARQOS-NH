...
<head>
    ...
    <style>
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #147a54; border-radius: 10px; }
        @keyframes pulse-slow { 0%, 100% { opacity: 0.4; } 50% { opacity: 0.2; } }
...
<body class="antialiased">
    ...
    <nav class="glass-nav sticky top-0 z-50 px-6 py-4">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div class="hidden md:flex items-center gap-8 text-xs font-bold uppercase tracking-widest text-gray-500">
                <a href="{{ route('welcome') }}#hero" class="hover:text-[#147a54] transition-colors">Beranda</a>
                <a href="{{ route('welcome') }}#fitur" class="hover:text-[#147a54] transition-colors">Fitur</a>
                <a href="{{ route('welcome') }}#sosial" class="text-[#147a54] transition-colors">Kegiatan Sosial</a>
                <a href="{{ route('welcome') }}#masjid" class="hover:text-[#147a54] transition-colors">Informasi Masjid</a>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="grid md:grid-cols-12 gap-12 items-start">
                ...
                <div class="md:col-span-4 sticky top-28">
                    <div class="glass-card p-3 rounded-[45px] shadow-2xl border-4 border-white transition-transform duration-700 hover:scale-[1.02]">
                        <div class="relative rounded-[35px] overflow-hidden bg-slate-200 shadow-inner" style="aspect-ratio: 2 / 3;">
                    ...
                    <div class="mt-8 p-8 bg-white rounded-[35px] border border-slate-100 shadow-sm">
                        ...
                        <div class="flex justify-between items-end mb-3">
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Progress Dana</span>
                ...
                <div class="md:col-span-8 space-y-10">
                    ...
                    <div class="glass-card p-10 rounded-[50px] border border-white/50 shadow-xl border-l-8 border-l-[#147a54]">
                        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-10 gap-6">
                            <div>
                                <h3 class="text-3xl font-black text-slate-900 tracking-tight">Donatur Dermawan</h3>
                                <p class="text-sm font-bold text-slate-400 mt-1 uppercase tracking-widest italic">Barakallahu Fiikum</p>
                            </div>
                            <div class="w-14 h-14 bg-green-50 text-[#147a54] rounded-2xl flex items-center justify-center">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[600px] overflow-y-auto pr-3 custom-scrollbar">
                            @forelse($donatur as $d)
                                <div class="p-6 bg-white/60 rounded-[30px] border border-slate-50 hover:border-green-200 transition-all flex justify-between items-center group">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 group-hover:bg-[#147a54] group-hover:text-white transition-colors">
                        ...
                        <div class="mt-12 text-center">
                            <a href="{{ route('welcome') }}" class="inline-flex items-center gap-3 text-[10px] font-black text-slate-400 hover:text-[#147a54] transition-all uppercase tracking-[0.3em]">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M10 19l-7-7m0 0l7-7m-7 7h18" stroke-width="3" stroke-linecap="round"/></svg>
                                Kembali ke Beranda
                            </a>
                        </div>
                    </div>
    ...
    <div id="modalDonasi" class="fixed inset-0 z-[60] hidden flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm shadow-inner" onclick="closeDonasiModal()"></div>
        <div class="glass-card relative w-full max-w-md p-10 rounded-[50px] shadow-2xl border border-white/20">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h3 class="text-3xl font-black text-slate-900 tracking-tight">Infaq Terbaik</h3>
                    <p id="namaKegiatanModal" class="text-[10px] text-[#147a54] font-black uppercase tracking-[0.2em] mt-2"></p>
                </div>
                <button onclick="closeDonasiModal()" class="w-10 h-10 flex items-center justify-center bg-slate-100 text-slate-400 rounded-full hover:bg-red-50 hover:text-red-500 transition-all">
            ...
            <form id="donasiForm" class="space-y-6">
                ...
                <div>
                    <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-3 ml-2">Nama Donatur</label>
                    <input type="text" id="modal_nama" placeholder="Hamba Allah" class="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/50 focus:ring-4 focus:ring-green-500/10 focus:border-[#147a54] outline-none text-sm font-bold transition-all placeholder:text-slate-300">
                </div>
                <div>
                    <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-3 ml-2">Nominal (Rp)</label>
                    <input type="number" id="modal_nominal" min="10000" placeholder="Contoh: 50000" class="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/50 focus:ring-4 focus:ring-green-500/10 focus:border-[#147a54] outline-none text-sm font-bold transition-all placeholder:text-slate-300" required>
                    <p class="text-[9px] text-slate-400 mt-2 ml-2 italic">* Minimal donasi Rp 10.000</p>
                </div>
                
                <div class="pt-6">
                    <button type="submit" id="pay-button" class="w-full py-5 bg-[#147a54] text-white font-black rounded-2xl shadow-xl shadow-green-900/20 hover:bg-[#0d5c3f] transition-all flex items-center justify-center gap-3 group">
